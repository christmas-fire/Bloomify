...
function FlowerCard({ flower, inCartCount, onAddToCart, onRemoveFromCart }) {
    ...
    return (
        <li style={{background:'#f6f7f9', boxShadow:'0 2px 12px rgba(0,0,0,0.06)', borderRadius:12, padding:'18px 18px 14px 18px', display:'flex', flexDirection:'column', gap:6, position:'relative'}}>
            ...
            <div style={{marginTop:'auto', display:'flex', justifyContent:'center', alignItems:'center', gap:8}}>
                {inCartCount > 0 ? (
                    <>
                        <button style={{background:'#e9fbe5', color:'#388e3c', border:'1.5px solid #b6e7b0', borderRadius:7, padding:'7px 18px', fontWeight:600, fontSize:'1.01rem', cursor:'default'}} disabled>
                            В корзине ({inCartCount})
                        </button>
                        <button style={{background:'#fff0f0', color:'#d32f2f', border:'1.5px solid #f3bcbc', borderRadius:7, padding:'7px 12px', fontWeight:600, fontSize:'1.01rem', cursor:'pointer'}} onClick={onRemoveFromCart}>
                            Убрать из корзины
                        </button>
                    </>
                ) : (
                    <button style={{background:'#e9fbe5', color:'#388e3c', border:'1.5px solid #b6e7b0', borderRadius:7, padding:'9px 22px', fontWeight:600, fontSize:'1.08rem', cursor:'pointer'}} onClick={onAddToCart}>
                        В корзину
                    </button>
                )}
            </div>
